<script>
    ...
    import {
        FolderOpen, Plus, X, Edit3, Trash2,
    ...
    async function saveGroup() {
        ...
        try {
            if (editingGroup) {
                await api.updateGroup(editingGroup.id, { name: formName.trim(), description: formDescription.trim() });
    ...
    async function searchResources() {
        if (!resourceSearch.trim()) { searchResults = []; return; }
        searching = true;
        try {
            searchResults = (await api.getResources({ search: resourceSearch, is_deleted: 0, limit: 10 })).items;
        } catch { searchResults = []; }
        searching = false;
    }
...
<div class="groups-page">
    ...
    {:else if groups.length === 0}
        <div class="empty">
            <FolderOpen size={48} />
            <p>暂无分组</p>
            <p class="hint">创建分组来整理你的资源，比如"电影合集"、"学习资料"</p>
        </div>
    {:else}
        <div class="group-list">
            {#each groups as group (group.id)}
                <div class="group-card" class:expanded={expandedId === group.id}>
                    <div class="group-row" on:click={() => toggleDetail(group)}>
                        <div class="group-info">
                            ...
                            <div class="group-meta">
                                <span class="resource-count">{group.resource_count || 0} 个资源</span>
                        ...
                        <div class="group-actions" on:click|stopPropagation>
                            <button class="btn-icon-sm" on:click={() => openEdit(group)} title="编辑"><Edit3 size={14} /></button>
                            <button class="btn-icon-sm danger" on:click={() => deleteGroup(group)} title="删除"><Trash2 size={14} /></button>
                            <ChevronRight size={16} class="expand-icon" class:rotated={expandedId === group.id} />
                        </div>
                    </div>
                        ...
                        <div class="group-detail">
                            ...
                            {:else if groupDetail}
                                ...
                                <div class="add-resource-section">
                                    <button class="btn-text" on:click={() => (showAddResource = !showAddResource)}>
                                        <Plus size={14} /> 添加资源到分组
                                    </button>
                                    {#if showAddResource}
                                        <div class="search-box">
                                            <input type="text" bind:value={resourceSearch} on:input={onSearchInput} placeholder="搜索资源..." />
                                            {#if searching}
                                                <span class="searching">搜索中...</span>
                                            {:else if searchResults.length > 0}
                                                <div class="search-results">
                                                    {#each searchResults as r}
                                                        <button class="result-item" on:click={() => addResourceToGroup(r.id)}>
                                                            <span>{r.title || "未命名资源"}</span>
                                                            <Plus size={12} />
                                                        </button>
                                                    {/each}
                                                </div>
                                    ...
                                    <div class="detail-resources">
                                        {#each groupDetail.resources as r}
                                            <div class="detail-resource-item">
                                                <a href={`/resource/${r.id}`} use:link class="resource-link">
                                                    {r.title || "未命名资源"}
                                                </a>
                                                <span class="resource-status">{r.status}</span>
                                                <button class="btn-icon-xs danger" on:click={() => removeResourceFromGroup(r.id)} title="移出分组">
                                                    <X size={12} />
                                                </button>
                                            </div>
                                        {/each}
    ...
    <div class="dialog-overlay" on:click|self={() => (showDialog = false)}>
        <div class="dialog">
            ...
            <div class="dialog-body">
                ...
                <label>
                    描述
                    <textarea bind:value={formDescription} placeholder="分组描述（可选）" rows="3"></textarea>
                </label>
            </div>
            <div class="dialog-footer">
                <button class="btn-cancel" on:click={() => (showDialog = false)}>取消</button>
                <button class="btn-submit" on:click={saveGroup} disabled={saving || !formName.trim()}>
                    {saving ? "保存中..." : "保存"}
                </button>
            </div>
        </div>
...
<style>
    .groups-page { max-width: 900px; margin: 0 auto; }
    ...
    .group-meta { display: flex; gap: 12px; margin-top: 4px; font-size: 12px; }
    .resource-count { color: #6366f1; }
    ...
    .expand-icon { color: #555; transition: transform 0.2s; }
    .expand-icon.rotated { transform: rotate(90deg); }

    .group-detail {
        border-top: 1px solid #222; padding: 16px 18px;
    }
    .loading-sm { color: #666; text-align: center; padding: 12px; font-size: 13px; }

    .add-resource-section { margin-bottom: 14px; }
    .btn-text {
        display: flex; align-items: center; gap: 4px;
        background: none; border: 1px dashed #333; color: #666;
        padding: 6px 14px; border-radius: 6px; font-size: 12px; cursor: pointer;
    }
    .btn-text:hover { border-color: #6366f1; color: #6366f1; }
    ...
    .searching { font-size: 12px; color: #666; display: block; margin-top: 4px; }
    .search-results { margin-top: 6px; display: flex; flex-direction: column; gap: 2px; max-height: 200px; overflow-y: auto; }
    .result-item {
        display: flex; align-items: center; justify-content: space-between;
        background: #1a1a1a; border: none; color: #aaa; padding: 8px 12px;
        border-radius: 6px; font-size: 13px; cursor: pointer; width: 100%; text-align: left;
    }
    .result-item:hover { background: #2a2a2a; color: #fff; }

    .detail-resources { display: flex; flex-direction: column; gap: 4px; }
    .detail-resource-item {
        display: flex; align-items: center; gap: 8px;
        padding: 8px 12px; background: #1a1a1a; border-radius: 6px;
    }
    .resource-link {
        flex: 1; font-size: 13px; color: #a0a0ff; text-decoration: none;
    }
    .resource-link:hover { color: #c0c0ff; }
    .resource-status {
        font-size: 11px; color: #555; padding: 1px 6px;
    ...
    .btn-icon-xs.danger:hover { color: #ef4444; background: #3a1a1a; }
    .no-resources { font-size: 13px; color: #555; padding: 8px 0; }
    ...
    .dialog-body label { display: flex; flex-direction: column; gap: 4px; font-size: 12px; color: #666; }
    .dialog-body input, .dialog-body textarea {
        background: #1a1a1a; border: 1px solid #333; color: #e0e0e0;
        padding: 8px 12px; border-radius: 8px; font-size: 14px; outline: none;
        resize: vertical;
    }
    .dialog-body input:focus, .dialog-body textarea:focus { border-color: #6366f1; }
